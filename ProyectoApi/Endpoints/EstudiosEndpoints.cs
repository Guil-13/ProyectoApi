...
namespace ProyectoApi.Endpoints
{
    public static class EstudiosEndpoints
    {
        ...
        public static RouteGroupBuilder MapEstudios(this RouteGroupBuilder group)
        {
            ...
            group.MapPost("/", Add).DisableAntiforgery().AddEndpointFilter<FiltroValidaciones<AddEstudioDTO>>();
            group.MapPut("/{id:int}", Update).DisableAntiforgery().AddEndpointFilter<FiltroValidaciones<AddEstudioDTO>>();
        ...
        static async Task<Results<Ok<List<Estudio>>,NoContent>> GetByUserId(int id, IRepositorio<Estudio> repositorio)
        {
